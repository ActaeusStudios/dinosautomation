function parseData(data){
		...
		$.ajax({
			   ...
			   success: function(response) {
			   	...
			   	for(i=0; i < hmi.dinos[thelocation].sensors.length; i++){
			   		$('#sensor_select').append("<option value='"+hmi.dinos[thelocation].sensors[i].name+"'>"+hmi.dinos[thelocation].sensors[i].friendlyName+"</option>");
			   	}
